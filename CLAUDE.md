...
| **opening ring (kuchiwa / 口輪)** | **The one exported part that is not part of the mold** — it stays in the finished lantern, which has no koma left in it once the mold is pulled out. A thin flat hoop (`ringGeometry`, `RING_WALL`/`RING_H` = 2mm) glued around the opening to hold it round. The **bottom** one additionally carries `LEG_N`=3 onigiri pads pointing inward, each bored ⌀`LEG_D`+fit, so the same part is the base the finished lamp stands on. Sized from `openingR()`, so it follows the design like every other part. |
