...
- **The washi template is not an output method, and has no download of its own.** It is not another way to make the mold — it is the paper skin you need on top of whichever mold you built, so it lives with the design settings (the 「和紙」 section: overlap / cover / panel size) and **rides along with whichever output you pick**: as `harigata_washi_a4.pdf` inside the kit ZIP for the 3D route, and as one more sheet among the cardboard template's pages (`paperParts` appends it) for the cardboard route. Do not re-add it to the output toggle: the toggle is mutually exclusive, and the washi is needed in both cases. Its panel width follows the rib count, so the cardboard copy is built from the *clamped* `pk.boards` = the mold that template actually makes.
