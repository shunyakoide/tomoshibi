...
- **`src/papercraft.js`** — **papercraft** (A4 full-scale 1:1 print pages for building the mold from cardboard/thick paper) and the **washi template** (`washiHTML`, the paper skin's own flat pattern). A pure module that derives the SVG from `geometry.js`'s 2D functions (see "Papercraft" / "Washi template" below). Both go through the same page shell `pagesHTML()` (ruler, registration marks, glue tabs, save-as-HTML), so the print rules can't drift apart between them.
...
- **The template also ships inside the kit ZIP** as a **PDF** (`harigata_washi_a4.pdf`, next to `harigata_config.json`) so it prints at 100% with no intermediate step; the same file is downloadable from the washi panel. Its panel width depends on the rib count being printed, and unlike the parts it cannot be re-derived from the STLs, so the ZIP would otherwise be an incomplete kit.
