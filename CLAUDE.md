...
| **opening ring (kuchiwa / 口輪)** | **The one exported part that is not part of the mold** — it stays in the finished lantern, which has no koma left in it once the mold is pulled out. A thin flat hoop (`ringGeometry`, `RING_WALL`/`RING_H` = 2mm) glued around the opening to hold it round, sized from `openingR()` so it follows the design like every other part. Top and bottom are the same hoop in two sizes, so **the bottom one carries one `MARK_D` bump on its inner rim** — purely a marker to tell the printed pair apart (the sphere preset's openings are ⌀60 vs ⌀56). The bump is a raised-cosine dip in the hole radius, not a step: a step puts near-coincident radii on one scanline and opens an edge. |
