...
One menu at the top right — on the phone's chip bar, and in the wide panel header, the same button in both. It holds what acts on the **app** or on the design **as a file**, plus the documents: the intro card, the build guide, the repository link, the language, backup save/restore, and reset. It replaced a `?` and a language toggle standing in the row itself.
...
- **A row with an `href` is a real `<a>`, not a button that calls `window.open`.** The repository link 「ソースコード (GitHub)」 is the only one so far. Middle-click, ⌘-click, "open in a new tab" and the screen reader's *link* role all come from the element and none of them from a click handler, and a popup blocker stops `window.open` while it never stops an anchor. The `↗` is drawn from the row's KIND rather than typed into the label, so every href row gets one and a translation cannot lose it. It stays in the first group, above the rule: the destructive 「初期化」 row is last, and it stays last.
  - **Why the app carries the link at all.** It is served from a static host with no install step and no about screen, so nothing else on the page says where it came from — and the licence and the issue tracker are both at the other end of it.
