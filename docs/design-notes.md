...
`CLAUDE.md` has the file map; this is why the map is shaped that way.
...
- **Everything WinAnsi cannot encode is drawn as filled outlines** from `pdf-glyphs.ts` (generated by
  `tools/pdffont` for exactly the characters the templates print — 24 of them, 12 kB; a whole CJK
