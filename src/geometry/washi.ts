...
/** How much a panel is grown past the bay it covers, and how many bays it covers. */
export type WashiOpts = { side?: number; end?: number; span?: number };
/**
 * One flattened panel. `outline` is the cut line; `marks` and `guides` are hints that are never cut.
 * `sTot` is the meridian ARC LENGTH (not the body height — see CLAUDE.md "Washi template"), and
 * `stretch` the worst flattening slack, which the page reports rather than scales away.
 */
