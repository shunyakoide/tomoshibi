 ...
 * It is deliberately NOT a Design — a preset says nothing about rib count or bamboo, and picking one
 * replaces only rTop/rBot/pts (see TomoshibiStudio's onPick). `height` is the one exception, and it
 * is optional because only a preset whose identity is a RATIO rather than an outline may claim it
 * (see 平丸 below); without it the maker's own height is kept.
 */
