...
import { UI, accent, mono, useT } from "./ui/theme.ts";
...
/** The folded detail block: a photograph (optionally a clip over it) and a paragraph, either of
 *  which may be missing — an empty one says the block is waiting, which is not a mistake. */
type MoreBlock = { photo?: string; video?: string; body?: string };
...
type Way = {
  ...
  needs?: (p: Design) => boolean; note?: string; detail?: SubStep[]; more?: MoreBlock;
};
...
type Step = {
  ...
  wip?: string; more?: MoreBlock; options?: Way[];
};
 ...
 */
function More({ more, t }: { more: MoreBlock; t: T }) {
  return (
 ...
 */
function Shot({ photo, video, t }: { photo?: string; video?: string; t: T }) {
  const [ok, setOk] = useState(true);
  const url = (f: string) => `${import.meta.env.BASE_URL}photos/${f}`;
  if (!photo || !ok) {
    return (
