 ...
 * the hand-rolled ZIP in stl.js). It takes the **same drawing-op list the SVG renderer consumes**
 * (papercraft.js `pageOps`), so the PDF and the printed HTML are literally the same drawing — the
 ...
 *   `pdf-glyphs.js`, extracted from an OFL font by `tools/pdffont` for exactly the characters the
 ...
 * ============================================================================
 */
import { GLYPHS } from "./pdf-glyphs.js";
...
const lat1 = (s) => { const u = new Uint8Array(s.length); for (let i = 0; i < s.length; i++) u[i] = s.charCodeAt(i) & 0xff; return u; };

// One op list → a PDF content stream (mm, y-down; the page CTM below does the flip).
function contentOf(ops, style) {
  const out = [];
  let cur = null;
  for (const op of ops) {
...
/**
 * Build the PDF. `pages` is a list of op lists (see papercraft.js `pageOps`), `page` is {w,h} in mm,
 * `style` is the shared style table. Returns the file as a Uint8Array.
 */
export function buildPDF(pages, page, style, title = "") {
  ...
  const chunks = [];
  let len = 0;
  const push = (s) => { const b = lat1(s); chunks.push(b); len += b.length; };
