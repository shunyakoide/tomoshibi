 ...
 * the requestAnimationFrame loop. What gets *drawn* is not here — scenes.ts rebuilds the contents
 ...
 * mold's shape comes from geometry.ts by way of scenes.ts.
 *
...
/**
 * The mutable handle every scene builder writes through. It is a plain object shared by reference
 * (see the note above), so the type is what says which fields exist — there is no class to read.
 * `setOrbit`/`setZoomRange` are attached below, once the orbit controls they close over exist.
 */
...
/**
 * What `useViewport` hands out: a live viewport, or the empty object it holds when WebGL failed to
 * initialize. The empty case is spelled as "every field, absent" rather than `{}` so that the one
 * check `buildScene` already made — `if (!s.group) return` — is what tells the two apart.
 */
...
// The low end is -1.35 because that is the print view's top-down pose (scenes.ts). It used to be
...
export function createViewport(mount: HTMLElement): { state: ViewportState; dispose: () => void } {
  ...
  // the lit view paints its own dark scene.background (see LIT_BG in scenes.ts).
  ...
  // Complete once the two methods below are attached — the controls they need do not exist yet.
  const state = {
