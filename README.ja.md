...
| 部品 | 役割 |
| --- | --- |
| **羽根板** | 型の面をつくる放射状の板（N枚、オレンジの房のように）。外縁が火袋の曲線で、竹ひごの座が並びます。両端に爪。3Dプリントの羽根板は内側が中央に向かってえぐってあり、それが乾いたあと開口から抜くための逃げになります。**段ボールの羽根板は内側が真っ直ぐ**です — その曲線は数百ミリのナイフ切りになるので。真っ直ぐでは抜けない形のときはパネルがその旨を出します。 |
| **コマ** | 歯車状のハブ、上下に同じものが2枚。縁のノッチが羽根板の爪を受けます。**段ボールルート**では継ぎ手を板ではなく段ボール用に寸法取りします — ノッチとノッチの間に残す壁は材料の厚みぶん、外径は開口より大きくてもよく、壁を厚くしても爪の差し込み量が減りません。板が厚いほどコマの芯は太り、その芯が羽根板の内側なので、開口ぎわに残る羽根板は材料が厚いほど細くなります。外径は**開口を越えません**。爪はそこに羽根板が持っている幅そのもので、爪の外縁と首の外縁が一本の線になります。その幅は開口の半径からコマの芯を引いた値で、初期の形・2mm板で13mm。板が厚いほど細り、段の間隔を下回ると警告が出ます。太い爪が要るなら口を広げることになり、そのため開口は⌀52より小さく描けません。 |
| **中間コマ** | **段ボールルート専用。** 胴が長く、両端のコマだけでは中ほどが支えられないときに入れるハブ（材料の厚みの隣のチェックボックス、初期値はオフ。枚数は高さから決まります）。段ボールでは端のコマと同じ部品です（このルートは爪に欠きを付けないので、ノッチが既に同じ深さまで届いています）。抜くときは羽根板に沿って滑らせるので、羽根板の内側が真っ直ぐである必要があります — 段ボールでは常に真っ直ぐです。真っ直ぐな内側で羽根板が開口から抜けない設計には入れられず、パネルがその旨を出します。3Dプリントの型には入りません。 |
| **土台** | 組んだ型を U 字サドル2つで受けて机から浮かせ、作業しながら回せるようにする台。3Dプリントのみ。 |
| **口輪** | 完成した提灯の2つの開口に貼る薄い平リング。コマを抜いたあとも開口の形を保ちます。**下側には脚ソケットを付けられます**（チェックボックス、初期値はオフ）— ⌀6 の穴を持つパッド3ヶ所で、脚は各自で用意します。開口が小さくて入らない場合は、内ふちに出っ張りを付けた輪のみになり、それが上下の見分けになります（開口はパッドが入る半径より小さく描けないので、この分岐は編集からは起きません）。 |
...
- **竹ひごの溝** — 木の型と同じように曲線に削り込んだ、等間隔ののこぎり歯です。先端は面の上にあり、竹ひごの上に被さります。竹ひごはその下の切れ込みに収まります。巻いた張力は輪を軸へ引きます。ただの溝は面が寝てくると外れていました — 壁の頂点が竹ひごより低くなるからで、返しのある溝なら外れません。垂直に近い面には歯はなく、ただの V の窪みです。**螺旋巻き**を選ぶと溝が羽根ごとにずれ、竹ひごが1本の連続した螺旋になります。このとき羽根板は1枚ずつ別形状になるので、通し番号を刻印した個別の STL として出ます。
...
青い線**です。土台はありません。下側のコマで自立します（組立ビューもその向きで表示します。
3Dプリントの型はこのルートが作らない土台に横倒しで載ります）。竹ひごの座は**切りません** —
2mm の溝は段ボールでは切るより先に潰れますし、竹ひごを留めているのは羽根板の縁ではなく和紙と
糸です。外周は滑らかな曲線のままで、竹ひごの位置を **5mm の横線**で示します。断面ビューと
組立ビューにも同じ横線を描きます — 竹ひごは胴を一周するので、断面では**左右**両側に出ます。
見えているものが鉛筆で印を付ける場所そのものです。
