...
- **和紙の型紙**（beta） — 提灯の紙そのものを平面に展開したもの。羽根板と羽根板の間の1面を原寸で出すので、貼ったあとに切るのではなく**貼る前に切れます**。どちらのルートを選んでも同梱されます。
...
- **☰ メニュー** — はじめかたのカード、作り方、言語、バックアップの保存・復元、初期化、そしてこのリポジトリへのリンク。
...
> どちらも印刷パーツと同じ関数から寸法を出していて自動で検証もしていますが、実際にこの方法で
> つくった人はまだ少ないです。粗いところがあるはずなので、合わなかったら issue を立ててください。
